      ...
      <div className="relative overflow-hidden flex-shrink-0" style={{ height: "150px" }}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[16px] font-black text-white tracking-tight">{ind.name}</h3>
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {ind.tags.map((tag) => (
            <span
              key={tag}
              className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${ind.accentBg} ${ind.accentText} ${ind.accentBorder} border`}
            >
              {tag}
            </span>
          ))}
